...
class CustomTextField: UITextField {
    ...
    let usernameIcon:UIImageView = {
        let icon = UIImageView(image: #imageLiteral(resourceName: "love").withRenderingMode(.alwaysTemplate))
        icon.tintColor = .white
        return icon
    }()

    lazy var userNameIconStack:UIStackView = {
        let stack = UIStackView(arrangedSubviews: [usernameIcon,userNameLineView])
        stack.axis = .vertical
        return stack
    }()
    
    lazy var userNameTextFieldStack:UIStackView = {
        let stack = UIStackView(arrangedSubviews: [userNameIconStack,self])
        return stack
    }()
}
...
class CustomTextFieldTest: UITextField {
    var isEditProfile:Bool?{
        didSet{
            if self.text != ""
            {
    ...
    func setupViews()
    {
        addSubview(textFieldSecondIcon)
        textFieldSecondIcon.anchorToView(top: topAnchor, leading: trailingAnchor, bottom: bottomAnchor, trailing: nil, padding: .init(top: 0, left: -35, bottom: 0, right: -35), size: .init(width:35,height:35), centerH: false, centerV: false)
        
        addSubview(customLabelPlaceHolder)
        self.addSubview(textFieldIcon)
        textFieldIcon.anchorToView(top: topAnchor, leading: leadingAnchor, bottom: bottomAnchor, trailing: nil, padding: .init(top: 0, left: 0, bottom: 1, right: 0), size: .init(width: 35, height: 35))
        customLabelPlaceHolder.anchorToView(leading: leadingAnchor, padding: .init(top: 0, left: 35, bottom: 0, right: 35))
        self.addTarget(self, action: #selector(startEditing), for: .editingDidBegin)
        self.addTarget(self, action: #selector(finishEditing), for: .editingDidEnd)
        //self.textAlignment = .center
        addSubview(bottomSeparator)
    ...
    var textFieldSecondIcon:UIImageView = {
    ...
    lazy var userNameIconStack:UIStackView = {
        let stack = UIStackView(arrangedSubviews: [textFieldIcon/*,userNameLineView*/])
        stack.axis = .vertical
        return stack
    }()
    ...
    override func textRect(forBounds bounds: CGRect) -> CGRect {
        return CGRect(x: bounds.origin.x +  40, y: bounds.origin.y, width: bounds.width - 70, height: bounds.height)
    }
